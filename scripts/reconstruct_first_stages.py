...
def reconstruct_and_save_images(input_dir, output_dir, model, image_size):
    for image_name in os.listdir(input_dir):
        if not image_name.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
            continue

        image_path = os.path.join(input_dir, image_name)
        image_tensor = load_and_preprocess_image(image_path, resize_shape=(image_size, image_size))
...
def main(config_path, ckpt_path, input_dir, output_dir, image_size):
    config = OmegaConf.load(config_path)
    model_info, step = load_model_from_config(config, ckpt_path)
    model = model_info["model"]

    os.makedirs(output_dir, exist_ok=True)
    reconstruct_and_save_images(input_dir, output_dir, model, image_size)
...
if __name__ == "__main__":
    ...
    main(args.config, args.ckpt, args.input_dir, args.output_dir, args.image_size)